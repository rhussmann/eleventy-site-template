...
STACK_ID=`aws cloudformation list-stacks | jq -r '.StackSummaries[] | select(.StackName == "eleventy-site-template" and .StackStatus == "CREATE_COMPLETE") | .StackId'`

aws s3 rb --force s3://$DOMAIN_NAME
aws cloudformation delete-stack --stack-name $STACK_ID

while [ `aws cloudformation describe-stacks --stack-name=$STACK_ID | jq -r '.Stacks[0].StackStatus'` != "DELETE_COMPLETE" ]; do
